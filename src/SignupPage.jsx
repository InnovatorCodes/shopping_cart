  ...
  async function signup(event) {
    ...
    async function addUser(newUser) {
      ...
      if (error && error.message.substring(0, 13) == "duplicate key") {
        ...
        const { error } = await supabase
          .from("User Carts")
          .insert([{ user_id: newUser.user_id, cart_items: { items: [] } }])
          .select();
        if (error) console.log(error);
      ...
      <div className="password">
        <input
          ...
          onChange={(event) => {
            ...
            if (touched && event.target.value != confPassword)
        ...
        >
          {showPassword ? hidePasswordBtn : showPasswordBtn}
        </div>
      </div>
      <div className="conf-password">
        <input
          ...
          onChange={(event) => {
            ...
            if (touched && event.target.value != password)
              setAlert("Passwords do not match");
            else setAlert("");
          }}
          placeholder=" "
        ...
        <div
          className="show-conf-password"
          onClick={() => setShowConfPassword(!showConfPassword)}
        >
          {showConfPassword ? hidePasswordBtn : showPasswordBtn}
        </div>
      </div>
