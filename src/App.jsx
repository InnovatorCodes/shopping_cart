...
function App() {
  ...
  function ShopPage(){
    ...
    return(
      <>
        ...
        {productCategories}
        <h2>All Products</h2>
        <ProductCards products={products} cart={cart} images={images} userID={loggedIn? user.user_id: null} loading={loading} setCurrentPage={setCurrentPage} filter={filter}></ProductCards>
      </>
    )
  }
  ...
  function SignupPage(){
    ...
    async function signup(event){
      ...
      if(password!=confPassword) setAlert('Passwords do not match')
      if(alert==''){
        ...
        <div className="password">
          <input type={showPassword?"text":"password"} 
            ...
            onChange={(event)=>{
              ...
              if(touched && event.target.value!=confPassword) setAlert('Passwords do not match');
              else setAlert('');
            }} 
            placeholder=' ' 
            required />
        ...
        <div className="conf-password">
          <input type={showConfPassword?"text":"password"} 
            ...
            onChange={(event)=>{
              ...
              if(touched && event.target.value!=password) setAlert('Passwords do not match');
              else setAlert('');
            }} 
            placeholder=' ' 
            required />
          <label htmlFor="confirm-password">Confirm Password</label>
          <div className="show-conf-password" onClick={()=>setShowConfPassword(!showConfPassword)}>{showConfPassword? hidePasswordBtn : showPasswordBtn}</div>
        </div>
        <div className="fullname">
          <input type="text" name="fullname" id="fullname" className={touched?'touched':''} onChange={(event)=>setFullName(event.target.value)} placeholder=' ' required />
  ...
  return (
    <>
      ...
      ( currentPage=='cart'? <CartPage cart={cart} setCart={setCart} products={products} user={user} setCurrentPage={setCurrentPage} loggedIn={loggedIn} images={images} />: 
      ( currentPage=='order placed'? <OrderPlacedPage />: 
      ( currentPage=='signup'? <SignupPage /> :<Login setLoggedIn={setLoggedIn} setUser={setUser} setCurrentPage={setCurrentPage} />) )))}
    </>
  )
}
